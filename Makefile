...
plan-buildkite: .env init workspace
	docker-compose run --rm envvars ensure --tags terraform
	docker-compose run --rm terraform-utils sh -c 'terraform workspace list; cd ${TERRAFORM_ROOT_MODULE}; echo "<h3>Root folder: ${TERRAFORM_ROOT_MODULE}</h3><b>Workspace: ${TERRAFORM_WORKSPACE}</b><br><b>Target AWS Account: ${AWS_ACCOUNT_ID}</b><div><pre><code>" >> ${TERRAFORM_ROOT_MODULE}-${TERRAFORM_WORKSPACE}.md; terraform plan -no-color >> ${TERRAFORM_ROOT_MODULE}-${TERRAFORM_WORKSPACE}.md; echo "</code></pre></div>" >> ${TERRAFORM_ROOT_MODULE}-${TERRAFORM_WORKSPACE}.md; sed -i 's/#/./' ${TERRAFORM_ROOT_MODULE}-${TERRAFORM_WORKSPACE}.md;'
